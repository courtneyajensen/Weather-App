...
async function getWeather(city) {
    var apiUrl =
    "https://api.openweathermap.org/data/2.5/weather?q=" + city + "&units=imperial&appid=9795009f60d5d1c3afe4e6df6002c319";
    ...
    if (response.ok) {
        ...
        var humidityValue = data.main.humiditiy;
        ...
        cityDataIcon.innerHTML = nameValue + currentDate.format(" (M/DD/YYYY) ") + icon;
...
async function uvIndex(lat, lon) {
    var uviUrl = "https://api.openweathermap.org/data/2.5/onecall?lat=" + lat + "&long=" + "&units=imperial&appid=9795009f60d5d1c3afe4e6df6002c319";
    ...
    if (response.ok) {
        ...
        if (uviValue >= 8) { uviLine.classList.add("badge", "badge-danger");}
        if (uviValue >= 6 && uviValue < 8) { uviLine.classList.add("badge", "bade-warning");}
        if (uviValue < 6 && uviValue >=3) { uviLine.classList.add("badge", "badge-success");}
        if (uviValue < 3) { uviLine.classList.add("badge", "badge-info");}
        ...
        for (var i = 0; i < fiveDayData.length; i++)
        {
            ...
            var icon = '<img src="${weatherURL}" style="width: 85px"/>';
            cardString += `
